...
public class Controller3  {
    ...
    public void setPerson(Sotr sotr) {
        this.sotr = sotr;

        famField.setText(sotr.getFam());
        okladField.setText(String.valueOf(sotr.getOklad()));
        otrDayField.setText(String.valueOf(sotr.getOtrday()));
        Id.setText(String.valueOf(sotr.getId()));
    ...
    public void handleOk(javafx.event.ActionEvent event) {
        ...
        if (isInputValid()) {
            ...
            try {
                DAO1 dao = new DAO1();
                dao.updateClient(sotr);
                mainController.updateTable();
                controller.DialogInfo("Данные сотрудника успешно изменены! ");




            }catch (Exception e){
                controller.DialogError("Не удалось изменить данные сотрудника!");
                e.printStackTrace();
            }
